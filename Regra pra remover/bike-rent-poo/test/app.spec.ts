...
import { UserWithRentError } from "../src/errors/user-with-rent-error"
...
describe('App', () => {
    ...
    it('should not let remove user that has any rent'),async () => {
        const app = new App(userRepo, bikeRepo, rentRepo)
        const appFake = new FakeRentRepo()
       const user = new User('jose', 'paulo@mail.com', '1234')
       const bike = new Bike('caloi mountainbike', 'mountain bike',
       1234, 1234, 100.0, 'My bike', 5, [])
       const rent = new Rent(bike,user,new Date())
       await appFake.add(rent)
       await expect(app.removeUser(user.email).rejects.toThrow(UserWithRentError))
    }
})