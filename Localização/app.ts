...
export class App {
    ...
    let valor;
    let time = new Date() - b.dateFrom; //new date seria a data atual quando a pessoa retornou
    return time*valor;
    }
    ...
    getLoc(latitude:number,longitude:number,bike:Bike){
        bike.coords.push(latitude)
        bike.coords.push(longitude)
    }

}
function diffHours(date1:Date,date2:Date) {
